...
Everything lives in [webapp/](./webapp/) — static HTML/JS, no build step. It runs MediaPipe Pose in Safari on the bedside phone and POSTs alerts to `ntfy.sh`. Default mode is **posture** (no zones): it detects lying → sitting (early warning) and sitting → standing/walking (main alert) wherever she is in frame, so one phone covers both the couch and the bed — just move it to whichever room she's resting in. A fixed-camera **bed zone** mode (tap-drawn polygon) is available in Settings. The default fast-response profile samples at 10 FPS and confirms sit-up, standing, or bed-zone exit in about half a second; loss of pose confirms in one second so a quick walk out of frame is still caught. A sitting-up concern re-alerts every 5 seconds, a missing/not-visible person re-alerts every 2 seconds, and a visible person who remains up re-alerts every 30 seconds. A "camera stopped working" warning fires if the video freezes, and a **Send test alert** button sits on the live monitor screen. Tests: `node --test webapp/test/*.test.mjs`.
